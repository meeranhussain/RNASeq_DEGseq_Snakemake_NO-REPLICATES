...
################ Fetech GTF for the mentioned organism ########################
# folder path
...
rule create_metatable:
    ...
    shell:"""
        mkdir -p {output}
        Rscript /mnt/data1/ShareApps/Automation_scripts/Reference_NoReplicates_DEGseq/create_combinations.R {input.master_file} {params.comb}
    """

rule DEGSeq_Analysis:
    input:
    ...
    shell:"""
        ...
        Rscript /mnt/data1/ShareApps/Automation_scripts/Reference_NoReplicates_DEGseq/DEGSeq_no_replicate_final.R {input.counts} '{params.org}' {params.org_code}
    """
