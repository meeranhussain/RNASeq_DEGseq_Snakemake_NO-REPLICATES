...
for (i in metatables) {
  ...
  #metatable <- read.table("2_Combinations/Treatment1_Treatment2", header = T)
  ...
  read_counts <- counts(dds)
  read_counts <- data.frame(Geneid = row.names(read_counts), read_counts)
  outputDir <- file.path(getwd(), "DEGexpresults")
  DEGexp(geneExpMatrix1=read_counts, geneCol1=1, expCol1=c(2), groupLabel1=ctrl, geneExpMatrix2=read_counts, geneCol2=1, expCol2=c(3), groupLabel2=trtd, method="LRT", pValue = 0.05, outputDir = outputDir)
  DEGresult<-read.table(file=file.path(outputDir,"output_score.txt"),header = T,sep = "\t")
  ...
  rownames(res_2) <- res_2$GeneNames
  ...
  b$Regulation[b$log2.Fold_change..normalized > 0.0] <- "UP"
  # if log2Foldchange < -0.0 and pvalue < 0.05, set as "DOWN"
  b$Regulation[b$log2.Fold_change..normalized < -0.0] <- "DOWN"
  b$Significance <- "No" 
  b$Significance[b$p.value < 0.05] <- "Yes"
  colnames(b)[1]  <- "Gene_IDs" 
  b <- b[-c(4,8,9)]
  colnames(b)[2:6] <- c(ctrl, trtd, "log2FoldChange", "pvalue", "qvalue") 
  ...
  ggsave(path = output_path_PLot, filename = paste(ctrl, trtd, "Volcanoplot.png", sep = "_"), device='png', dpi=700)
  ######################################################################################################################
  
  ############################################### SCATTERED PLOT ##########################################################
  ggplot(b, aes(x= log10(b[,2]), y=log10(b[,3]), col=Expression)) + theme_minimal(base_size = 16) + geom_point(size = 4) + 
    scale_colour_manual(values = mycolors) + ggtitle(title) + xlab(trtd) + ylab(ctrl) + theme(plot.title = element_text(hjust = 0.5))  
  ggsave(path = output_path_PLot, filename = paste(ctrl, trtd, "Scatteredplot.png", sep = "_"), device='png', dpi=700)
  #######################################################################################################################
  ...
  # rownames(df.top) <- df.top$Gene_IDs
  # head(df.top)
  # dim(df.top)
  # #Filtered rlog based on significant genes HeatMap
  # fil_genes <- df.top[1:50,1:3]
  # fil_genes <- fil_genes[-1]
  # 
  ...
  sig_up_down_top50[2][sig_up_down_top50[2] == "0"] <- 0+1
  
  sig_up_down_top50[3][sig_up_down_top50[3] == "0"] <- 0+1
  ############################################################################################################################
  
  heatmap_ann <- as.data.frame(colData(dds)[,c(1,2)])
  row.names(heatmap_ann) <- heatmap_ann$condition
  #HeatMap of normalized count
  output_path_heatmapPLot <- file.path("3_Analysis", "2_Deseq2", "Plots/", ctrl)
  ##Sample Based heatmap
  pheatmap(log2(sig_up_down_top50[2:3]), cluster_rows=T, color = colorRampPalette(c("green1", "black", "red"))(20), show_rownames=T,
           cluster_cols=T, annotation_col=heatmap_ann, border_color = "black", cellwidth = 100, treeheight_row = 200, filename = paste(output_path_heatmapPLot, trtd, "Heatmap.png", sep = "_"))
  
  #pheatmap(log10(sig_up_down_top50[2:3]), cluster_rows=T, color = colorRampPalette(c("green2", "black", "red2"))(20), show_rownames=T,
           #cluster_cols=T, annotation_col=heatmap_ann)
  #############################################################################################################################
  ...
  GSEA_bar_Kegg <- barplot(kegg_ORA, 
          drop = TRUE, 
          showCategory =30, 
          title = paste(ctrl, trtd, sep = " vs "),
          font.size = 10)
  ggplot2::ggsave(file.path(Kegg_ORA_Bar_path), plot = GSEA_bar_Kegg, device='png', dpi=1000, width = 10, height = 10)
